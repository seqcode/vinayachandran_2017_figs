...
cp ../shared_files/Xu_2009_ORF_TSS_TES_V64.gff Xu_ORF_CUT_SUT_XUT.gff
cat ../shared_files/Xu_2009_CUTs_TSS_ONLY_V64.gff >> Xu_ORF_CUT_SUT_XUT.gff
cat ../shared_files/Xu_2009_SUTs_TSS_ONLY_V64.gff >> Xu_ORF_CUT_SUT_XUT.gff
cat ../shared_files/van_Dijk_2011_XUTs_V64_TSS_ONLY.gff >> Xu_ORF_CUT_SUT_XUT.gff
bedtools sort -i Xu_ORF_CUT_SUT_XUT.gff > Xu_ORF_CUT_SUT_XUT_sorted.gff

bedtools closest -a tss_nodups_sorted.gff -b Xu_ORF_CUT_SUT_XUT_sorted.gff -d -D a -id -io > upstream_opposite_strand.gff
cat upstream_opposite_strand.gff | awk '$19 < 500 {print $0}' > upstream_opposite_strand_within_500bp.gff
python check_for_divergent.py

#create list of all TSSs
cat divergent_list.tsv | awk '$2 != "None" {print $2}' > divergent_only.txt
python ../Sup4/genes_from_gff.py ../shared_files/Xu_2009_ORF_TSS_TES_V64_sorted.gff divergent_only.txt 
cp tss_nodups.gff all_tss.gff
cat divergent_only.gff >> all_tss.gff

cat tss_nodups.gff | awk '{print $9}' > tss_nodups_names.txt
cp tss_nodups_names.txt all_tss_names.txt
cat divergent_only.txt >> all_tss_names.txt
